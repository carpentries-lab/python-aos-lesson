...
where `arg1`, `arg2` ... might be input and ouput files and `options` might be `--color=red` or `-d`. Arguments `arg1`, `arg2` are called
positional arguments -- their role is determined by their position in the argument sequence. The other options are named and thus can
be supplied by the user in any order. Using named options is recommended when your script takes many arguments as it would be tedious
...
> 
> (Hint: Define the named option gridlines to be of type `bool` and give it a default value.)
>
> > ## Solution
> >
...
> > named options. The name of the option will be autogenerated. The short version of the option takes the first letter
> > of the variable name, `-g` for gridlines. The long version is `--gridlines`.
